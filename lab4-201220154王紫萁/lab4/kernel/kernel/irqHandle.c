...
void syscallStoreGlobal(struct StackFrame* sf);
void syscallFetchGlobal(struct StackFrame* sf);
...
void syscallHandle(struct StackFrame* sf) {
    ...
    case SYS_PID:
        ...
        break;
    case SYS_STG:
        syscallStoreGlobal(sf);
        break;
    case SYS_FEG:
        syscallFetchGlobal(sf);
        break;
    default: break;
    }
}
...
void blockOnStdIn() {
    ...
    pcb[current].sleepTime = 0x7fffffff;
}
...
void syscallFork(struct StackFrame* sf) {
    ...
    if (i != MAX_PCB_NUM) {
		...
		*/
        ...
        pcb[i].prevStackTop = (uint32_t) & (pcb[i].stackTop) - ((uint32_t) & (pcb[current].stackTop) - pcb[current].prevStackTop);
        pcb[i].state = STATE_RUNNABLE;
...
void syscallFetchGlobal(struct StackFrame* sf) {
    disableInterrupt();
    pcb[current].regs.eax = sharedVariable;
    enableInterrupt();
}