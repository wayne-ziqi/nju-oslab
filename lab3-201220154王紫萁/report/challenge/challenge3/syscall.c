...
int32_t syscall(int num, uint32_t a1, uint32_t a2,
	...
	*/
    ...
    asm volatile("movl %%eax, %0"
                 : "=m"(eax)); //store general registers' value
    ...
    asm volatile("movl %%eax, %0"
                 : "=m"(ret)); //treat eax as return value
...
int exec(int argc, char** argv) {
    /*TODO:call syscall*/
    return syscall(SYS_EXEC, (uint32_t)argc, (uint32_t)argv, 0, 0, 0);
    // return 0;
}
...
uint32_t pthread_create(uint32_t func) {
    return syscall(SYS_THREAD_CREATE, func, 0, 0, 0, 0);
}

uint32_t pthread_destroy() {
    return syscall(SYS_THREAD_DESTROY, 0, 0, 0, 0, 0);
}
