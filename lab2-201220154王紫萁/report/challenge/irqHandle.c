...
//refractor the keyboard irq and print irq
void basicPrint(const char character)
{
	if (character == '\n')
	{ // 回车符
		// 处理回车情况
...
void KeyboardHandle(struct TrapFrame *tf)
{
	...
	{ // 退格符
		...
		if (displayCol > 0 && displayCol > tail && bufferTail > 0)
		{
			displayCol--;
			bufferTail--;
	...
	else
	{
		...
		else if (code < 0x81 && (character = getChar(code)) != 0)
		{
			keyBuffer[bufferTail++] = character;
			bufferTail %= MAX_KEYBUFFER_SIZE;
			putChar(character);
		}
		basicPrint(character);
	}
