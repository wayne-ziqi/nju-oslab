...
static void setIntr(struct GateDescriptor *ptr, uint32_t selector, uint32_t offset, uint32_t dpl) {
	...
	*/
	...
	// else if(dpl == 3) ptr->segment = USEL(selector);
	// else assert(dpl == 0 || dpl == 3);
...
void initIdt() {
	...
	// setTrap(idt + 0x9, SEG_KCODE, (uint32_t)irqErrorCode, DPL_KERN);
	...
	// setTrap(idt + 0xf, SEG_KCODE, (uint32_t)irqErrorCode, DPL_KERN);
	// setTrap(idt + 0x10, SEG_KCODE, (uint32_t)irqErrorCode, DPL_KERN);
	...
	setIntr(idt + 0x20 + 1, SEG_KCODE, (uint32_t)irqKeyboard, DPL_KERN);
